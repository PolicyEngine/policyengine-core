...
def test_dump(tax_benefit_system):
    ...
    testing.assert_array_equal(
        simulation.household.members_role, simulation_2.household.members_role
    )
...
def test_dump_restore_with_memberless_group_entity(tax_benefit_system):
    directory = tempfile.mkdtemp(prefix="openfisca_")
    simulation = SimulationBuilder().build_from_entities(
        tax_benefit_system,
        {
            "persons": {"Alice": {"salary": {"2017-01": 2000}}},
            "households": {"h1": {"parents": ["Alice"]}, "h2": {}},
        },
    )
